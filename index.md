...
3、禁止具有管理中心访问权限的用户登陆，之前有那么一段时间有只禁全局的情况下，现在是全禁。
——老E3 SY常见翻车形式，大部分添加过Microsoft 365 A3 - Unattended License for students use benefit的翻车方式。

4、提示“用户将在2019年10月1日(太平洋时间)禁用账户“，用户在此之前可以进行数据的转移，这是2019年部分教育全局出现的状况，以后估摸着不可能出现了。
——2019年部分教育全局。
#### 新操作
1、SharePoint 访问报错
```markdown
